...
import { Zap, Wrench, Cog, Rocket, Brain, Flame, Star, Target, Cpu, Copy } from 'lucide-react';
  ...
  const contextFields = {
    invention: [
      { 
        ...
        label: 'Core Concept', 
        icon: Brain,
        tooltip: 'The main idea or technology you want to develop (e.g., "AI-powered exoskeleton", "Holographic display system")'
      },
      { 
        key: 'creator', 
        label: 'Mad Inventor', 
        icon: Wrench,
        tooltip: 'Your inventor name or the project lead (e.g., "Dr. Sarah Chen", "Team Alpha")'
      },
      { 
        key: 'category', 
        label: 'Innovation Field', 
        icon: Cog,
        tooltip: 'The scientific/tech domain (e.g., "Robotics", "Biotech", "Quantum Computing", "Materials Science")'
      },
      { 
        key: 'constraints', 
        label: 'Current Limits', 
        icon: Target,
        tooltip: 'Known limitations or requirements (e.g., "Must be portable", "Budget: $1M", "Power efficient")'
      }
    ],
    experiment: [
      { 
        key: 'hypothesis', 
        label: 'Wild Hypothesis', 
        icon: Zap,
        tooltip: 'Your experimental theory or question (e.g., "Quantum tunneling in room temperature", "DNA storage systems")'
      },
      { 
        key: 'researcher', 
        label: 'Visionary', 
        icon: Wrench,
        tooltip: 'Lead researcher or team name (e.g., "Prof. Martinez", "Quantum Lab Team")'
      },
      { 
        key: 'field', 
        label: 'Science Domain', 
        icon: Cog,
        tooltip: 'Research field (e.g., "Particle Physics", "Neuroscience", "Astrophysics", "Chemistry")'
      },
      { 
        key: 'resources', 
        label: 'Available Arsenal', 
        icon: Rocket,
        tooltip: 'Equipment and resources available (e.g., "Electron microscope, clean room", "Supercomputer access")'
      }
    ]
  };
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="mb-8 text-center">
          <div className="flex items-center justify-center mb-4">
            ...
            <Zap className="h-12 w-12 text-yellow-400 animate-pulse ml-4" />
          </div>
                    ...
                    return (
                      <div key={field.key} className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-12 pr-4 py-3 bg-gray-800/50 border border-cyan-500/30 rounded-xl text-white placeholder-gray-400 focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300 hover:border-cyan-400/50"
                        />
                        
                        {/* Tooltip */}
                        {activeTooltip === field.key && (
                          <div className="absolute z-50 bottom-full left-0 mb-2 w-80 max-w-sm">
                            <div className="bg-black/90 backdrop-blur-sm border border-cyan-500/50 rounded-lg p-3 shadow-2xl">
                              <div className="flex items-start space-x-2">
                                <Zap className="h-4 w-4 text-yellow-400 mt-0.5 flex-shrink-0" />
                                <div>
                                  <p className="text-cyan-300 font-semibold text-sm mb-1">{field.label} Guide:</p>
                                  <p className="text-gray-300 text-xs leading-relaxed">{field.tooltip}</p>
                                </div>
                              </div>
                              {/* Tooltip arrow */}
                              <div className="absolute top-full left-4 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-cyan-500/50"></div>
                            </div>
                          </div>
                        )}
                      </div>
